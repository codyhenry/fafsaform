...
CREATE TABLE form.fafsas
(
    id SERIAL PRIMARY KEY,
    account_id INT NOT NULL REFERENCES form.accounts ON DELETE CASCADE,
...
CREATE TABLE form.schools
(
    school_id SERIAL PRIMARY KEY,
    school_name VARCHAR(100)
)
CREATE TABLE form.housing_plans
(
    id SERIAL PRIMARY KEY,
    housing_plan VARCHAR(15) NOT NULL UNIQUE CHECK (housing_plan IN ('on campus', 'off campus', 'with parents')),
)
CREATE TABLE form.application_types
(
    id SERIAL PRIMARY KEY,
    application_type VARCHAR(15) NOT NULL UNIQUE CHECK (application_type IN ('regular decision','early decision', 'early action'))
)
CREATE TABLE form.users_schools
(
    user_id INT NOT NULL REFERENCES form.users ON DELETE CASCADE,
    school_id INT NOT NULL REFERENCES form.schools ON DELETE RESTRICT,
    housing_plan_id INT REFERENCES form.housing_plans ON DELETE RESTRICT,
    application_type_id INT REFERENCES form.application_types ON DELETE RESTRICT,
    PRIMARY KEY (user_id, school_id)
)
CREATE TABLE form.user_categories
(
    id SERIAL PRIMARY KEY,
    user_type VARCHAR(20) NOT NULL UNIQUE CHECK (category IN ('student', 'custodial', 'noncustodial'))
)
CREATE TABLE form.users
(
    id SERIAL PRIMARY KEY,
    fafsa_id INT NOT NULL REFERENCES form.fafsas ON DELETE CASCADE,
    first_name VARCHAR(50) NOT NULL,
    last_name VARCHAR(50) NOT NULL,
    date_of_birth DATE NOT NULL CHECK (date_of_birth <= CURRENT_DATE),
    category_id INT NOT NULL REFERENCES form.categories ON DELETE RESTRICT
)
CREATE TABLE form.marital_statuses (
    id SERIAL PRIMARY KEY,
    marital_status VARCHAR(20) NOT NULL UNIQUE CHECK (marital_status IN ('never married', 'married', 'divorced','widowed'))
)
CREATE TABLE form.high_school_statuses
(
    id SERIAL PRIMARY KEY,
    high_school_status VARCHAR(20) NOT NULL UNIQUE CHECK (high_school_status IN ('diploma', 'ged', 'home schooled','none'))
)
CREATE TABLE form.fafsaStudentDetails
(
    user_id INT PRIMARY KEY REFERENCES form.users ON DELETE CASCADE,
    ...
    marital_status_id INT NOT NULL REFERENCES form.marital_statuses ON DELETE RESTRICT,
    high_school_status_id INT NOT NULL REFERENCES form.high_school_statuses ON DELETE RESTRICT,
...
CREATE TABLE form.users_address
(
    user_id INT NOT NULL REFERENCES form.users,
    address_id INT NOT NULL REFERENCES form.address ON DELETE CASCADE,
    PRIMARY KEY (user_id, address_id)
)
CREATE TABLE form.addresses
(
    id SERIAL PRIMARY KEY,
...
CREATE TABLE form.relationships_to_student
(
    id SERIAL PRIMARY KEY,
...
CREATE TABLE form.highest_education_levels
(
    id SERIAL PRIMARY KEY,
    highest_education_level VARCHAR(50) NOT NULL UNIQUE CHECK (highest_education IN ('middle school', 'high school', 'college','unknown'))
)
CREATE TABLE form.fafsaParentDetails
(
    user_id INT PRIMARY KEY REFERENCES form.users ON DELETE CASCADE,
    relationship_to_student_id INT REFERENCES form.relationships_to_student ON DELETE RESTRICT,
    social_security VARCHAR(10) NOT NULL,
    phone VARCHAR(11) NOT NULL,
    email VARCHAR(100) NOT NULL,
    ...
    legal_residence_date DATE NOT NULL CHECK (legal_residence_date <= CURRENT_DATE),
    highest_education_id INT NOT NULL REFERENCES form.highest_education_levels ON DELETE RESTRICT,
    is_veteran BOOLEAN NOT NULL,
...
CREATE TABLE form.cssStudentDetails
(
    user_id INT PRIMARY KEY REFERENCES form.users ON DELETE CASCADE,
...
CREATE TABLE form.cssParentDetails
(
    user_id INT PRIMARY KEY REFERENCES form.users ON DELETE CASCADE,
...
CREATE TABLE form.governmentAssistance
(
    user_id INT PRIMARY KEY REFERENCES form.users ON DELETE CASCADE,
...
CREATE TABLE form.taxReturns
(
    user_id INT PRIMARY KEY REFERENCES form.users ON DELETE CASCADE,
    is_manual BOOLEAN NOT NULL,
    form_link TEXT
)
CREATE TABLE form.housing_situations
(
    id SERIAL PRIMARY KEY,
...
CREATE TABLE form.households
(
    user_id INT PRIMARY KEY REFERENCES form.users ON DELETE CASCADE,
    housing_situation_id NOT NULL REFERENCES form.housing_situations,
...
CREATE TABLE form.business_types
(
    id SERIAL PRIMARY KEY
    business_type VARCHAR(20) NOT NULL UNIQUE CHECK (business_type IN ('corporation', 'partnership', 'sole proprietorship'))
)
CREATE TABLE form.business_tax_locations
(
    id SERIAL PRIMARY KEY,
    tax_location VARCHAR(100) NOT NULL UNIQUE CHECK (tax_location IN ('schedule c', 'schedule e','1065','1120','1120-s','other'))
)
CREATE TABLE form.businesses
(
    user_id INT PRIMARY KEY REFERENCES form.users ON DELETE CASCADE,
    ...
    business_type_id NOT NULL REFERENCES form.business_types ON DELETE RESTRICT,
    ...
    tax_form_location_id INT NOT NULL REFERENCES form.business_tax_locations ON DELETE RESTRICT,
    gross_receipts INT NOT NULL CHECK (gross_receipts >= 0),
    total_expenses INT NOT NULL CHECK (total_expenses >= 0)
)
-- CREATE TABLE form.taxReturnsManual
-- (
--     id SERIAL PRIMARY KEY,
-- )
...
-- ENUMS
INSERT INTO form.business_tax_locations (tax_location) VALUES ('schedule c'), ('schedule e'), ('1065'), ('1120'), ('1120-s'), ('other');
INSERT INTO form.business_types (business_type) VALUES ('corporation'), ('partnership'), ('sole proprietorship');
INSERT INTO form.housing_situations (housing_situation) VALUES ('own home'), ('rent home'), ('live with others'), ('housing provided');
INSERT INTO form.relationships_to_student (relationship_to_student) VALUES ('mother'), ('father'), ('step mother'), ('step father'), ('sibling'), ('stepsibling'), ('spouse'), ('child'), ('grandparent'), ('other');
INSERT INTO form.highest_education_levels (highest_education_level) VALUES ('middle school'), ('high school'), ('college'), ('unknown');
INSERT INTO form.marital_statuses (marital_status) VALUES ('never married'), ('married'), ('divorced'), ('widowed');
INSERT INTO form.user_categories (user_type) VALUES ('student'), ('custodial'), ('noncustodial');
INSERT INTO form.application_types (application_type) VALUES ('regular decision'), ('early decision'), ('early action');
INSERT INTO form.housing_plans (housing_plan) VALUES ('on campus'), ('off campus'), ('with parents');
INSERT INTO form.high_school_statuses (high_school_status) VALUES ('diploma'), ('ged'), ('home schooled'), ('none');
